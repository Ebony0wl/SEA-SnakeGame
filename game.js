...
function drawApple(){
  /* TO DO */
  document.getElementById('apple').src = "https://www.pikpng.com/pngl/m/332-3329577_drawing-apples-chibi-cartoon-apples-transparent-background-clipart.png";
  context.drawImage(document.getElementById('apple'), apple.x, apple.y, grid, grid);
...
function drawSnake(){
  /* TO DO */
    for(let i=0; i < snake.cells.length; i++){
      //let curr = snake.cells[i];
      if(i===0){
        drawCellWithBitmoji(snake);
        
      }else{
        context.fillStyle = snake.color;
        context.fillRect(snake.cells[i].x, snake.cells[i].y, grid-10, grid-10);
        context.fill();
...
function checkCrashItself(){
  /* TO DO */
  for (let i=0; i<snake.cells.length; i++){
    for(let j=i+1; i<snake.cells.length; j++){
